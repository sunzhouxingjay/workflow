...
public class TaskEntityImpl extends VariableScopeImpl implements TaskEntity, Serializable, BulkDeleteable {
  ...
  //记得要增加字段
  protected String userId="anyone";
  ...
  protected transient boolean isIdentityLinksInitialized;
  ...
  protected transient boolean isCanceled;
  protected String eventName;
  protected transient ActivitiListener currentActivitiListener;
  ...
  protected List<VariableInstanceEntity> loadVariableInstances() {
    //sunzhouxing:自己加的
    // List<VariableInstanceEntity> variableList= Context.getCommandContext().getDbSqlSession().findVariableInstanceEntitiesByTaskId(id);
    // if (variableList.isEmpty()) {
    //   //variableList=Context.getCommandContext().getVariableInstanceEntityManager().findVariableInstancesByTaskId(id);
    // }
    // return variableList;
  ...
  public void setName(String taskName) {
    this.name = taskName;
    String allocatedUser=workflowContext.getUserTaskBindHandler().getUser(oid, taskName);
    if (allocatedUser!=null) {
      /**
       * todo:无论是没有分配还是未绑定都是返回的null
       * 这一块没有必须需要分配，是为了方便智慧城市的使用
       * 但是没有分配时应该有一个默认值，现在的默认值比较简单
       */
      setUserId(allocatedUser);
    }
    // //自己增加的根据taskName与commandcontext中的allocationTable,设置userId
    // ConcurrentHashMap<String,String> table=allocationTable.getAllocationTable();
    // if (table!=null) {
    //   if(table.containsKey(taskName)) setUserId(table.get(taskName));
    //   else throw new RuntimeException("init task userId error, cause by allocationTable do not exist userTask:"+taskName);
    // }
  }
